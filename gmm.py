...
class Mesh:
    ...
    def v_degree(self):
        vd = np.zeros((len(self.edges), 2), dtype=float)
        for i, e in enumerate(self.edges):
            vd[i][0] = 1/(len(self.ve[e[0]]) ** 0.5)
            vd[i][1] = 1/(len(self.ve[e[1]]) ** 0.5)
        return vd
...
edge_attr = torch.tensor(i_mesh.vd, dtype=torch.float, requires_grad=True)
edge_attr = torch.cat([edge_attr, edge_attr[:,[1,0]]], dim=0)
...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.conv4 = GMMConv(hidden_size[2], hidden_size[3], 2, n_kernels)
        self.conv5 = GMMConv(hidden_size[3], hidden_size[4], 2, n_kernels)
        self.conv6 = GMMConv(hidden_size[4], hidden_size[5], 2, n_kernels)
        self.conv7 = GMMConv(hidden_size[5], hidden_size[6], 2, n_kernels)
        self.conv8 = GMMConv(hidden_size[6], hidden_size[7], 2, n_kernels)
        self.conv9 = GMMConv(hidden_size[7], hidden_size[8], 2, n_kernels)
        self.conv10 = GMMConv(hidden_size[8], hidden_size[9], 2, n_kernels)
    ...
    def forward(self, data):
        x, edge_index, edge_attr = dataset.x.to(device), dataset.edge_index.to(device), dataset.edge_attr.to(device)

        dx = self.conv1(x, edge_index, edge_attr)
        dx = self.bn1(dx)
        dx = self.l_relu(dx)

        dx = self.conv2(dx, edge_index, edge_attr)
        dx = self.bn2(dx)
        dx = self.l_relu(dx)

        dx = self.conv3(dx, edge_index, edge_attr)
        dx = self.bn3(dx)
        dx = self.l_relu(dx)

        dx = self.conv4(dx, edge_index, edge_attr)
        dx = self.bn4(dx)
        dx = self.l_relu(dx)
        ...
        dx = self.conv9(dx, edge_index, edge_attr)
        dx = self.bn9(dx)
        dx = self.l_relu(dx)

        dx = self.conv10(dx, edge_index, edge_attr)
        dx = self.bn10(dx)
        dx = self.l_relu(dx)
